    ...
    servers.each do |machine|
        ...
        # config.vm.define machine[:hostname] do |node|
            ...
            if box_file_in.nil? || box_file_in.empty?
                ...
                node.vm.provision "shell", inline: "export DEBIAN_FRONTEND=noninteractive; sudo launchpad-getkeys; sudo apt-get update -y" # launchpad-getkeys ensures that if a port is blocked the keys will be imported properly before apt update
                ...
                # node.vm.provision "shell", inline: "sudo apt-get install network-manager -y" # Used for DNS
                # Check env
