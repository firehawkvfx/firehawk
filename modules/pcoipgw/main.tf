...
resource "null_resource" "pcoipgw" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      ansible-playbook -i "$TF_VAR_inventory" ansible/newuser_deadlineuser.yaml -v --extra-vars "variable_host=pcoipgw_eip.0 hostname=gateway.${var.public_domain_name} pcoip=true set_hostname=true variable_connect_as_user=centos variable_user=deployuser variable_uid=$TF_VAR_deployuser_uid set_selinux=disabled" --tags 'onsite-install'; exit_test
