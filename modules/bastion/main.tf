...
resource "null_resource" "provision_bastion" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      ansible-playbook -i "$TF_VAR_inventory" ansible/get-file.yaml -v --extra-vars "source=/var/log/cloud-init-output.log dest=$TF_VAR_firehawk_path/tmp/cloud-init-output-bastion.log variable_user=centos variable_host=bastion"; exit_test
EOT
