...
resource "null_resource" "provision_node_centos" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ansible-playbook -i "$TF_VAR_inventory" ansible/deadline-localworkstation-install.yaml --tags "onsite-install" --skip-tags "multi-slave" --extra-vars "variable_host=role_node_centos variable_user=centos"; exit_test
