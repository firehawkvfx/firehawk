...
resource "null_resource" "provision_softnas_volumes" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      ansible-playbook -i "$TF_VAR_inventory" ansible/newuser_deadlineuser.yaml -v --extra-vars "variable_host=role_softnas variable_connect_as_user=$TF_VAR_softnas_ssh_user variable_user=deployuser"; exit_test
      ...
      # ansible-playbook -i "$TF_VAR_inventory" ansible/newuser_deadlineuser.yaml -v --extra-vars 'variable_host=firehawkgateway variable_connect_as_user=deployuser variable_user=deployuser'; exit_test
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      if [[ $TF_VAR_remote_mounts_on_local == true ]] ; then
        ...
        ansible-playbook -i "$TF_VAR_inventory" ansible/linux-volume-mounts.yaml -v -v --extra-vars "variable_host=workstation1 variable_user=deadlineuser ansible_ssh_private_key_file=$TF_VAR_onsite_workstation_private_ssh_key" --skip-tags 'cloud_install local_install_onsite_mounts' --tags 'local_install'; exit_test
      fi
EOT
  }
}
