...
resource "null_resource" "provision_softnas" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      ansible-playbook -i "$TF_VAR_inventory" ansible/get-file.yaml -v --extra-vars "source=/var/log/cloud-init-output.log dest=/firehawk/tmp/cloud-init-output-softnas.log variable_user=ec2-user variable_host=role_softnas"; exit_test
