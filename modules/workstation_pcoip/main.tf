...
EOT
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ansible-playbook -i "$TF_VAR_inventory" ansible/deadline-worker-install.yaml -v --tags "onsite-install" --extra-vars "variable_host=role_workstation_centos variable_user=centos variable_connect_as_user=centos"
