...
sed -i 's/^TF_VAR_taint_single=.*$/TF_VAR_taint_single=(module.softnas.aws_network_interface.nas1eth0[0] module.softnas.aws_instance.softnas1[0])/' $config_override # taint vpn
sed -i 's/^TF_VAR_allow_prebuilt_softnas_ami=.*$/TF_VAR_allow_prebuilt_softnas_ami=false/' $config_override # allow use of a prebuilt ami with the update if it exists.  if set to false, it will not use it, and it will replace the ami after the update